...
describe('readFiles utility function', () => {
    ...
    beforeAll(async () => {
        ...
        // Create test files
        await writeFile(join(testDir, 'valid.yml'), 'name: John\nage: 30\nskills:\n  - JavaScript')
        await writeFile(join(testDir, 'valid.yaml'), 'test: value\nconfig: true')
        await writeFile(join(testDir, 'valid.json'), '{"name": "Jane", "type": "config"}')
        await writeFile(join(testDir, 'invalid.yml'), 'name: John\nage: [invalid yaml structure')
    ...
    it('should handle invalid YAML gracefully', async () => {
        ...
        expect(result['invalid.yml']).toHaveProperty('error', 'Failed to parse YAML')
    ...
    it('should handle multiple files at once', async () => {
        const result = await readFiles(['valid.yml', 'valid.json', 'plain.txt'], testDir)
    ...
    it('should handle non-existent files gracefully', async () => {
        ...
        expect(typeof result).toBe('object')
        expect(result['does-not-exist.yml']).toHaveProperty('error', 'Failed to read file')
