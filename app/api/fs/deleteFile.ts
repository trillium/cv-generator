...
export async function deleteFile(options: DeleteFileOptions) {
    const { filePath, baseDirectory = process.env.PII_PATH || '.', createBackup = true } = options
...
async function createBackupFile(originalFilePath: string, content: string): Promise<boolean> {
    try {
