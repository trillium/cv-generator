...
describe('writeFile utility function', () => {
    ...
    it('should write YAML file successfully', async () => {
        const testData = { name: 'John', age: 30, skills: ['JavaScript', 'TypeScript'] }
    ...
    it('should handle invalid base directory', async () => {
        ...
        const result = await writeFile(testData, {
            filePath: 'test.yml',
            baseDirectory: '/completely/invalid/path/that/does/not/exist/and/cannot/be/created',
            createDiff: false
        })
    ...
    it('should handle null and empty data', async () => {
        const result = await writeFile(null as any, {
