...
import { getFilesInDirectory, getAllFilesRecursively, getAllFiles } from './getFiles'
...
describe('getFiles utility functions', () => {
    ...
    beforeAll(async () => {
        ...
        await writeFile(join(resumesDir, 'subfolder', 'resume2.pdf'), 'another fake pdf')
    })
    ...
    describe('getFilesInDirectory', () => {
        it('should return only files from a directory', async () => {
        ...
        it('should handle non-existent directory gracefully', async () => {
            const result = await getFilesInDirectory('/tmp/does-not-exist-' + Date.now())
    ...
    describe('getAllFilesRecursively', () => {
        it('should recursively find all files', async () => {
            const result = await getAllFilesRecursively(resumesDir, testDir)
            expect(result).toContain('resumes/resume1.pdf')
            expect(result).toContain('resumes/subfolder/resume2.pdf')
        })

        it('should handle non-existent directory gracefully', async () => {
            const result = await getAllFilesRecursively('/tmp/does-not-exist-' + Date.now())
            expect(result).toEqual([])
        })
    ...
    describe('getAllFiles', () => {
        it('should combine main directory files and resumes subdirectory files', async () => {
            const result = await getAllFiles(testDir)

            expect(result.allFiles).toContain('data.yml')
            expect(result.allFiles).toContain('config.json')
            expect(result.allFiles).toContain('readme.txt')
            expect(result.allFiles).toContain('resumes/resume1.pdf')
            expect(result.allFiles).toContain('resumes/subfolder/resume2.pdf')
            expect(result.mainDirFiles).toBe(3) // data.yml, config.json, readme.txt
            expect(result.resumeFiles).toBe(2) // resume1.pdf, resume2.pdf
