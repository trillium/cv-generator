...
export async function writeFile(json: Object, options: WriteFileOptions) {
    const { filePath, baseDirectory = process.env.PII_PATH || '.', createDiff = true } = options
 ...
 */
export async function writeYamlFile(yamlContent: string, options: WriteFileOptions) {
    const { filePath, baseDirectory = process.env.PII_PATH || '.', createDiff = true } = options
