...
describe('Numbered Array Files', () => {
  describe('parseNumberedArrayFile', () => {
    ...
    it('returns null for invalid patterns', () => {
      ...
      expect(parseNumberedArrayFile('experience.workExperience.yml')).toBeNull()
      expect(parseNumberedArrayFile('experience.workExperienceABC.yml')).toBeNull()
    })

    it('returns null for unsupported extensions', () => {
      expect(parseNumberedArrayFile('experience.workExperience01.txt')).toBeNull()
      expect(parseNumberedArrayFile('experience.workExperience01.js')).toBeNull()
    })
  })
  ...
  describe('mergeNumberedArrayFiles', () => {
    ...
    it('merges numbered files in correct order', () => {
      ...
      fs.writeFileSync(file1, yaml.dump({ workExperience: [{ position: 'Job 1' }] }))
      fs.writeFileSync(file2, yaml.dump({ workExperience: [{ position: 'Job 2' }] }))
    ...
    it('sorts files by number even if provided out of order', () => {
      ...
      fs.writeFileSync(file1, yaml.dump({ workExperience: [{ position: 'Job 1' }] }))
      fs.writeFileSync(file2, yaml.dump({ workExperience: [{ position: 'Job 2' }] }))
      fs.writeFileSync(file3, yaml.dump({ workExperience: [{ position: 'Job 3' }] }))
    ...
    it('throws error if file contains non-array data', () => {
      ...
      expect(() => mergeNumberedArrayFiles([file1])).toThrow('must contain an array')
    })
    ...
    it('sorts files by numeric value, not string order', () => {
      ...
      fs.writeFileSync(file1, yaml.dump({ workExperience: [{ position: 'Job A' }] }))
      fs.writeFileSync(file2, yaml.dump({ workExperience: [{ position: 'Job B' }] }))
      fs.writeFileSync(file3, yaml.dump({ workExperience: [{ position: 'Job C' }] }))
  ...
  describe('validateNumberedArrayFiles', () => {
    ...
    it('validates correct numbered array files', () => {
      ...
      expect(() => validateNumberedArrayFiles([file1, file2], tmpDir)).not.toThrow()
    })
    ...
    it('allows non-consecutive numbering', () => {
      ...
      expect(() => validateNumberedArrayFiles([file1, file2, file3], tmpDir)).not.toThrow()
    })

    it('throws error for numbered file with wrong section', () => {
      const file1 = path.join(tmpDir, 'experience.workExperience01.yml')

      fs.writeFileSync(file1, yaml.dump({ workExperience: [{}], education: [] }))

      expect(() => validateNumberedArrayFiles([file1], tmpDir)).toThrow('must only contain')
    })
    ...
    it('throws error for numbered file with non-array data', () => {
      ...
      expect(() => validateNumberedArrayFiles([file1], tmpDir)).toThrow('must contain an array')
    })
