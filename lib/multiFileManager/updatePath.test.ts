...
describe('updatePath YAML array syntax', () => {
  ...
  it('should write arrays with dash syntax, not numeric object keys', async () => {
    ...
    await updatePath('resumes/test-company', 'workExperience[0].position', 'Software Developer')
  ...
  it('should preserve dash syntax when updating existing array items', async () => {
    ...
    await updatePath('resumes/test-company', 'workExperience[0].position', 'Senior Developer')
  ...
  it('should write nested arrays with dash syntax', async () => {
    ...
    await updatePath('resumes/test-company', 'workExperience[0].details[0].subhead', 'HackForLA')
      ...
      projects: [
        {
          ...
          lines: [{ text: 'First bullet point' }, { text: 'Second bullet point' }],
        },
      ],
    })
  ...
  it('should handle numbered array files correctly', async () => {
    ...
    await updatePath('resumes/test-company', 'projects[1].lines[1].text', 'BULLET CHANGED')
  ...
  it('should handle files with multiple items correctly when calculating adjustedIndex', async () => {
    ...
    fs.writeFileSync(
      project01,
      yaml.dump({
        projects: [{ name: 'First Project', lines: [{ text: 'First bullet' }] }],
      }),
    )
  ...
  it('should not create phantom null array items when updating nested paths in numbered files', async () => {
    ...
    fs.writeFileSync(
      ...
      yaml.dump({
        projects: [
          {
            name: 'First Project',
            lines: [{ text: 'First bullet' }, { text: 'Second bullet' }, { text: 'Third bullet' }],
          },
        ],
      }),
    )
    ...
    await updatePath('resumes/test-company', 'projects[1].lines[2].text', 'UPDATED THIRD LINE')
