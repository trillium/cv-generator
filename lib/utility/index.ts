...
export function extractCopyData(yamlPath: string, parsedData: CVData): unknown {
  ...
  function withLLM(obj: unknown): unknown {
    ...
    if (parsedData.llm && obj && typeof obj === 'object' && !Array.isArray(obj)) {
  ...
  if (arrayFields.includes(pathParts[0] as keyof CVData) && pathParts.length === 1) {
  ...
  if (arrayFields.includes(pathParts[0] as keyof CVData) && pathParts.length >= 2) {
    ...
    function isWorkExperienceLike(obj: unknown): obj is {
      ...
      return !!obj && typeof obj === 'object' && 'position' in obj && 'company' in obj
    }
    const idx = Number(pathParts[1])
    if (!Number.isNaN(idx) && arr && arr[idx]) {
      ...
      if (
        ...
        if (!Number.isNaN(lineIdx) && isWorkExperienceLike(arr[idx])) {
          const item = arr[idx]
