...
describe('File Manager Utility Functions', () => {
    ...
    describe('readResumeData', () => {
        it('should successfully read resume data from files', async () => {
            const mockFileData = {
                ...
                'frontend.yml': {
                    info: { firstName: 'Jane', lastName: 'Smith', email: 'jane@example.com' },
                    workExperience: []
                }
            }
    ...
    describe('deleteResumeWithBackup', () => {
        it('should successfully delete resume with backup', async () => {
            ...
            expect(result.success).toBe(true)
            expect(result.data).toEqual(mockResponse)
            expect(mockFetch).toHaveBeenCalledWith('/api/fs/delete?filePath=data.yml&createBackup=true', {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        it('should delete without backup when specified', async () => {
            ...
            expect(result.success).toBe(true)
            expect(mockFetch).toHaveBeenCalledWith('/api/fs/delete?filePath=data.yml&createBackup=false&baseDirectory=%2Fcustom', {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    describe('Integration scenarios', () => {
        it('should handle workflow: list -> read -> save -> duplicate', async () => {
            ...
            mockFetch.mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve({ files: { 'data.yml': { info: { firstName: 'John' } } } })
            })
            ...
            mockFetch.mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve({ filePath: '/data.yml', diffCreated: true })
            })

            const saveResult = await saveResumeChanges('data.yml', readResult.data!['data.yml'])
            expect(saveResult.success).toBe(true)

            // Duplicate
            mockFetch.mockResolvedValueOnce({
                ok: true,
                json: () => Promise.resolve({ sourcePath: '/data.yml', destinationPath: '/backup.yml' })
            })
    ...
    describe('listAllResumeFiles API Integration', () => {
        it('should successfully fetch real file list from API', async () => {
            try {
                ...
                console.log('⚠️  Server may not be running on port 4444. Skipping integration test.')
                console.log('   To run integration tests, start the dev server with: pnpm dev')
            }
        ...
        it('should validate API response structure matches interface', async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    const expectedFields = ['allFiles', 'mainDirFiles', 'resumeFiles', 'totalFiles']
                    const missingFields = expectedFields.filter(field => !(field in rawData))
                    const extraFields = Object.keys(rawData).filter(field => !expectedFields.includes(field) && field !== 'success' && field !== 'directory')
