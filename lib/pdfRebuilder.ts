...
export function wasRecentlyRebuilt(directoryPath: string, withinMs: number): boolean {
...
export async function rebuildPdfs(
  ...
  console.log(`[${timestamp}] 🔄 Triggering PDF regeneration for: ${directoryPath}`)
  ...
  const processes = pdfsToRegenerate.map((pdfType) => {
    ...
    return new Promise<{ pdfType: string; success: boolean; error?: string }>((resolve) => {
      child.on('close', (code) => {
        if (code === 0) {
          console.log(`✅ ${pdfType} PDF generation completed`)
          resolve({ pdfType, success: true })
        } else {
          console.error(`❌ ${pdfType} PDF generation failed with code ${code}`)
  ...
  Promise.all(processes).then((results) => {
    ...
    if (allSucceeded) {
      console.log(`✅ All PDF generation completed successfully for job ${pdfJobId}`)
      pdfJobTracker.completeJob(pdfJobId, {})
    } else {
      const failures = results.filter((r) => !r.success)
      console.error(`❌ PDF generation failed for: ${failures.map((f) => f.pdfType).join(', ')}`)
