...
export type SectionIndex = Record<string, ItemIndex>
export type LibraryIndex = Record<string, SectionIndex>

export function scanLibrary(libraryPath: string): LibraryIndex {
...
export function scanManifestUsage(resumesPath: string): Map<string, Set<string>> {
...
export function generateIndex(piiPath: string): LibraryIndex {
  ...
  const usageMap = scanManifestUsage(resumesPath)
