...
export class ResumeIndexManager {
    ...
    addResumeVersion(metadata: ResumeMetadata, resumePath: string): void {
        ...
        if (company) {
            ...
            index.positions[position].companies[company] =
                index.positions[position].companies[company].filter(
                    v => v.date !== companyVersion.date
                );
    ...
    removeResumeVersion(position: string, company?: string, date?: string): void {
        ...
        if (!company) {
            ...
            if (companyVersions) {
                index.positions[position].companies[company] =
                    companyVersions.filter(v => v.date !== date);
     ...
     */
    getRecentlyModified(limit: number = 10): { position: string; company: string; version: CompanyVersion }[] {
        const index = this.readIndex();
        const allVersions: { position: string; company: string; version: CompanyVersion }[] = [];
        ...
        return allVersions
            .sort((a, b) => new Date(b.version.lastModified).getTime() - new Date(a.version.lastModified).getTime())
            .slice(0, limit);
    }
     ...
     */
    getResumePath(position: string, company?: string, date?: string): string | null {
        const index = this.readIndex();
        const positionGroup = index.positions[position];
