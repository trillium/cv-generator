 ...
 */
export function getSectionMapping(yamlPath: string): SectionMapping | undefined {
 ...
 */
export function getSectionsForRoute(routeType: 'resume' | 'cover-letter'): SectionMapping[] {
  const routeFilters = {
    resume: ['info', 'careerSummary', 'header', 'workExperience', 'projects', 'profile'],
    'cover-letter': ['info', 'header', 'coverLetter', 'profile'],
  }

  return routeFilters[routeType].map((path) => SECTION_MAPPINGS[path]).filter(Boolean)
}
