...
export async function getHeaderResumeLines(headerId: number): Promise<DbHeaderResumeLine[]> {
  const db = await getDb()
...
export async function getHeaderTitleLines(headerId: number): Promise<DbHeaderTitleLine[]> {
  const db = await getDb()
  const query = db.query(
...
export async function getProfileLines(profileId: number): Promise<DbProfileLine[]> {
  const db = await getDb()
  const query = db.query('SELECT * FROM profile_lines WHERE profile_id = ? ORDER BY order_index')
  return query.all(profileId) as DbProfileLine[]
}

export async function getProfileLinks(profileId: number): Promise<DbProfileLink[]> {
  const db = await getDb()
  const query = db.query('SELECT * FROM profile_links WHERE profile_id = ? ORDER BY order_index')
  return query.all(profileId) as DbProfileLink[]
}

export async function getCareerSummary(resumeId: number): Promise<DbCareerSummaryItem[]> {
  const db = await getDb()
  const query = db.query(
...
export async function getCoverLetter(resumeId: number): Promise<DbCoverLetterLine[]> {
  const db = await getDb()
  const query = db.query(
