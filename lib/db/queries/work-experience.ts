...
import type { DbWorkExperience, DbWorkExperienceBubble, DbWorkExperienceLine } from '../types'

export async function getWorkExperiences(resumeId: number): Promise<DbWorkExperience[]> {
  const db = await getDb()
  const query = db.query('SELECT * FROM work_experiences WHERE resume_id = ? ORDER BY order_index')
