...
describe('API Throttling', () => {
  ...
  it('should show the difference between throttling and debouncing', () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      console.log(`  Throttle: ${throttleAllowed ? 'EXECUTE' : 'SKIP'} (count: ${throttleCount})`)
    ...
    console.log('   • Throttling: Executes at regular intervals, ignores rapid calls')
    console.log('   • Debouncing: Waits for silence, then executes once')
    console.log('   • For API calls: Throttling prevents overload, Debouncing prevents spam')
