...
export function getResumeByPosition(
  position: string,
): DefaultResumeResult | null {
  const piiPath = getPiiDirectory();
  const indexPath = join(piiPath, "resume-index.json");
...
export function getResumeByCompany(
  position: string,
  company: string,
): DefaultResumeResult | null {
  const piiPath = getPiiDirectory();
...
export function listAllResumes(): DefaultResumeResult[] {
  ...
  for (const [positionName, positionEntry] of Object.entries(
    index.positions || {},
  )) {
    if (positionEntry.default) {
      results.push({
    ...
    for (const [companyName, companyResumes] of Object.entries(
      positionEntry.companies || {},
    )) {
