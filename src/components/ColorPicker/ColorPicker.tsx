...
import type { ColorPickerProps, TailwindColor } from './types'
...
export default function ColorPicker({
  ...
  const handleColorClick = (colorName: string, shade: string) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      const colorFamily = (colors as unknown as Record<string, Record<string, string>>)[colorName]
      if (colorFamily && typeof colorFamily === 'object') {
        document.documentElement.style.setProperty('--color-primary', colorFamily[shade] || hex)
        document.documentElement.style.setProperty('--color-primary-50', colorFamily['50'] || hex)
        document.documentElement.style.setProperty('--color-primary-100', colorFamily['100'] || hex)
        document.documentElement.style.setProperty('--color-primary-200', colorFamily['200'] || hex)
        document.documentElement.style.setProperty('--color-primary-300', colorFamily['300'] || hex)
        document.documentElement.style.setProperty('--color-primary-400', colorFamily['400'] || hex)
        document.documentElement.style.setProperty('--color-primary-500', colorFamily['500'] || hex)
        document.documentElement.style.setProperty('--color-primary-600', colorFamily['600'] || hex)
        document.documentElement.style.setProperty('--color-primary-700', colorFamily['700'] || hex)
        document.documentElement.style.setProperty('--color-primary-800', colorFamily['800'] || hex)
        document.documentElement.style.setProperty('--color-primary-900', colorFamily['900'] || hex)
        document.documentElement.style.setProperty('--color-primary-950', colorFamily['950'] || hex)
      } else {
      ...
      console.log('🚨 DEBUG: --color-primary:', computedStyle.getPropertyValue('--color-primary'))
      console.log(
    ...
    const ColorPickerModal = () => (
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-primary-700 dark:text-primary-200 mt-1">{modalDescription}</p>
        </div>

        <ColorPicker
          selectedColor={selectedColor}
          onColorSelect={(color) => {
            console.log(`🚨 DEBUG: Modal color selected: ${color.name}-${color.shade}`)
    ...
    <div className={clsx('space-y-4', className)}>
              ...
              {COLOR_SHADES.map((shade) => {
                ...
                return (
                  <button
                    ...
                    onMouseEnter={() => setHoveredColor(`${colorName}-${shade}`)}
                    ...
                    className={clsx(
                      ...
                      {
                        'border-gray-400 dark:border-gray-500': !isSelected && !isHovered,
                        'border-blue-500 shadow-lg': isSelected,
                        'border-gray-600 dark:border-gray-300': isHovered && !isSelected,
                      },
                    )}
      ...
      {selectedColor && (
        <div className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-800 rounded-md">
          <div className="text-sm text-primary-700 dark:text-primary-200">Selected:</div>
