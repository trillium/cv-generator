...
const Header = ({ name, title, resume, omitTitle = false, omitBlurb = false }: HeaderProps) => {
        ...
        <div className="my-1">
          {title.map((line, index) => (
            <EditableField
              ...
              fieldType="text"
            >
              <p className="m-0 text-xl leading-1.5 dark:text-white">{line}</p>
            </EditableField>
          ))}
        </div>
      )}
        ...
        <div className="mt-2">
          {resume.map((line, index) => (
            <EditableField
              ...
              fieldType="textarea"
            >
              <p className="m-0 text-md leading-[1.3] dark:text-white">{line}</p>
            </EditableField>
          ))}
        </div>
      )}
