...
import type { CVData, Education, TechnicalCategory } from '@/types'
...
export const ProfileHeader = ({ lines, links, shouldDisplayProfileImage }: Data['profile']) => {
  return (
    <div className="flex flex-col items-startr ">
      {shouldDisplayProfileImage && <ProfileImage circular={true} border={true} />}
      <Title text="Contact" />
...
const ProfileSkills = ({ technical }: { technical: TechnicalCategory[] }) => {
  ...
  const technicalData =
    Array.isArray(technical) && technical.length > 0 ? technical : [{ category: '', bubbles: [] }]
  ...
  return (
    <div className="flex flex-col gap-2 w-full">
      ...
      {technicalData.map((tech, index) => {
        return (
          <div key={`${tech.category || 'default'}-${index}`} className="flex flex-col gap-1">
            <EditableField
            ...
            <div className="flex flex-wrap gap-1">
              {tech.bubbles && tech.bubbles.length > 0 ? (
                ...
                <EditableField yamlPath={`technical.${index}.bubbles.0`} value="" fieldType="text">
          ...
          return (
            <div className="flex flex-col gap-0.5" key={index}>
              ...
              <EditableField
                ...
                fieldType="text"
              >
                <span className="text-xs text-neutral-500 dark:text-neutral-400">{edu.years}</span>
              </EditableField>
            </div>
          )
        })}
