...
import { getNestedValue, useYamlPathUpdater } from '@/hooks/useYamlPathUpdater'
import ActionButtons from './ActionButtons'
import EditModal from './EditModal'
import EmptyFieldPlaceholder from './EmptyFieldPlaceholder'
import { isFieldEmpty, shouldShowAddButtons } from './editableFieldUtils'
...
export default function EditableField<T extends string | string[]>({
  ...
  const { parsedData, error, storedPdfMetadata, documentType } = useDirectoryManager()
  const { openModal, closeModal } = useModal()
  const { handleAddAbove, handleAddBelow, handleDelete, handleMoveUp, handleMoveDown } =
  ...
  const pdfMeta =
    documentType === 'resume' ? storedPdfMetadata?.pdf?.resume : storedPdfMetadata?.pdf?.coverLetter

  const lastPageLines = pdfMeta?.lastPageLines
  const pages = pdfMeta?.pages

  const { shouldHighlight, isFirstOverflow } = detectPageOverflow(children, lastPageLines)
  ...
  const hasLinkTag = (node: ReactNode): boolean => {
    return React.Children.toArray(node).some((child) => {
      if (React.isValidElement(child)) {
        ...
        if (hasChildrenProp(el.props) && el.props.children) return hasLinkTag(el.props.children)
      }
      return false
    })
  }
  ...
  if (!yamlPath || typeof yamlPath !== 'string') {
    console.warn('EditableField: yamlPath is required but was not provided or is invalid')
  ...
  const handleSave = async (modalEditValue: string | string[] | unknown[]) => {
    ...
    if (JSON.stringify(modalEditValue) !== JSON.stringify(currentValue)) {
      ...
      try {
        ...
        const revertValue = Array.isArray(value) ? value.join('\n') : String(value || '')
  ...
  const HighlightOverlay = () => (
    <div
      ...
      title={hasOverflow ? `Warning: This content appears on page ${pages}` : undefined}
    />
  )
