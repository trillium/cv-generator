...
export async function POST(request: NextRequest) {
  try {
    const { sourceFilePath, sectionKeys, targetFileName, mergedData } = await request.json()

    if (!sourceFilePath || !sectionKeys || !targetFileName) {
      return NextResponse.json(
        {
          success: false,
          error: 'Missing required fields: sourceFilePath, sectionKeys, and targetFileName',
        },
        { status: 400 },
      )
    }
