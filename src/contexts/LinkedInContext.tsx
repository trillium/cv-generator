...
import type { LinkedInData } from '@/types/linkedin'
...
export function LinkedInProvider({ children }: LinkedInProviderProps) {
  const [currentLinkedInData, setCurrentLinkedInData] = useState<LinkedInData | null>(null)
  const [currentLinkedInFile, setCurrentLinkedInFile] = useState<string | null>(null)
  ...
  const loadLinkedInFile = useCallback(async (dirPath: string) => {
    ...
    try {
      const response = await fetch(`/api/directory/load?path=${encodeURIComponent(dirPath)}`)
      if (!response.ok) {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to load LinkedIn data'
  ...
  const saveLinkedInData = useCallback(
    async (filePath?: string) => {
      ...
      try {
        ...
        const errorMessage = err instanceof Error ? err.message : 'Failed to save LinkedIn data'
      ...
      try {
        ...
        if (currentLinkedInFile) {
          try {
            ...
            const errorMessage = err instanceof Error ? err.message : 'Failed to save YAML content'
